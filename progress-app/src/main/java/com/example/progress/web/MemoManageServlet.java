...
public class MemoManageServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    if ("/memo/new".equals(servletPath)) {
      ...
      // 作成時刻は常に現在時刻を使用（入力値は無視）
      String created = nowString();
      String detail = nvl(req.getParameter("memoDetail"));
      String tags = nvl(req.getParameter("memoTags"));
      boolean pinned = "on".equalsIgnoreCase(nvl(req.getParameter("memoPinned"))) || "true".equalsIgnoreCase(nvl(req.getParameter("memoPinned")));
      if (!title.isEmpty()) {
        Memo m = new Memo(0, type, title, created, detail, null, emptyToNull(tags), pinned);
    ...
    if ("/memo/edit".equals(servletPath)) {
      ...
      String tags = nvl(req.getParameter("memoTags"));
      boolean pinned = "on".equalsIgnoreCase(nvl(req.getParameter("memoPinned"))) || "true".equalsIgnoreCase(nvl(req.getParameter("memoPinned")));
      String updated = nowString();
      Memo m = new Memo(memoId, type, title, created, detail, updated, emptyToNull(tags), pinned);
  ...
  private static String nowString() { return java.time.LocalDateTime.now().toString().replace('T',' ').substring(0,16); }
}
